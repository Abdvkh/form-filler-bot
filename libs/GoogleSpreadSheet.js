...
function checkOptions(options) {
   if (!options) {
      throwError("Need pass options")
   }
   if (!options.sheetName) {
      throwError("Need pass sheetName")
   }
   if (!options.onSuccess) {
      throwError("Need pass onSuccess command name")
   }
}
...
function getHeader(options) {
   ...
   let qrow = ""
   if (options.rowIndex) {
      qrow = "&rowIndex=" + options.rowIndex
   }
...
function postRow(options, isEdit) {
   ...
   if (!options.row) {
      throwError("Need pass table row data")
   }
...
function onSuccess() {
   ...
   if (!result) {
      ...
      return Bot.runCommand(errCalback, {
         error: error
      });
   }
